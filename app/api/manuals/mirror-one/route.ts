...
export async function POST(request: NextRequest) {
  try {
    ...
    const manual = data as ManualRecord

    if (!manual.manual_url) {
      return NextResponse.json(
        { success: false, error: 'Manual URL missing', slug },
        { status: 400 }
      )
    }
    ...
    const response = await fetch(manual.manual_url, {
      redirect: 'follow',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          error: `Failed to fetch external manual: ${response.status}`,
