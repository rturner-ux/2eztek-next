...
async function verifyRecaptcha(token: string): Promise<boolean> {
  ...
  try {
    ...
    return true // Non-fatal — allow through if verification fails
  }
}

export async function POST(req: Request) {
  const body = await req.json()
  const { plan, ownerEmail, ownerName, businessName, nonce, recaptchaToken } = body
  ...
  if (!human) {
    return NextResponse.json({ success: false, message: 'Security check failed. Please refresh and try again.' }, { status: 400 })
  }
