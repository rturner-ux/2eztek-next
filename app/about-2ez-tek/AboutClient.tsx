...
const timeline = [
  { year: '2016', event: '2EZ TEK established in Dallas Fort Worth as a fitness equipment service company.' },
  { year: '2020', event: 'Expanded to serve commercial gyms, apartment fitness centers, hotels, and corporate facilities.' },
  { year: '2025', event: 'Launched SmartGymOps to power service tracking, equipment history, and QR reporting.' },
  { year: 'Today', event: '10,000+ machines serviced. 500+ five-star reviews. Full DFW coverage.' },
]

const featuredReviews = googleReviews.slice(0, 6)
    ...
    <main className="min-h-screen overflow-hidden bg-white text-slate-900">
        ...
        <div className="mx-auto max-w-7xl grid gap-16 lg:grid-cols-2 lg:items-center">
          <Reveal direction="left">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">Who We Are</span>
            </div>
            <h2 className="text-4xl font-black leading-tight md:text-6xl">
              Dallas Fort Worth's
              <span className="block text-slate-400">Fitness Equipment Experts.</span>
            </h2>
                ...
                {timeline.map((item, i) => (
                  <div key={item.year} className="flex gap-5">
                    <div className="flex flex-col items-center">
                      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-cyan-200 bg-cyan-50 text-xs font-black text-cyan-600">{i + 1}</div>
                      {i < timeline.length - 1 && <div className="mt-2 h-full w-px bg-slate-200" />}
                    </div>
                    <div className="pb-6">
                      <div className="text-xs font-black uppercase tracking-[0.2em] text-cyan-600">{item.year}</div>
                      <p className="mt-2 text-sm leading-relaxed text-slate-500">{item.event}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-16 text-center">
            <div className="flex items-center justify-center gap-3 mb-6">
              <span className="h-px w-8 bg-cyan-400" />
              <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">Meet the Founder</span>
              <span className="h-px w-8 bg-cyan-400" />
            </div>
          </Reveal>
          ...
          <div className="grid gap-16 lg:grid-cols-[340px,1fr] lg:items-start">
            ...
            <Reveal direction="left">
              <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
                <div className="h-44 w-44 overflow-hidden rounded-full border-2 border-cyan-200 shadow-[0_0_40px_rgba(34,211,238,0.15)]">
                  <img
                ...
                <h2 className="mt-6 text-3xl font-black text-slate-900">Robby Turner</h2>
                <p className="mt-1 text-sm font-black uppercase tracking-[0.2em] text-cyan-600">Founder & CEO</p>

                <div className="mt-8 w-full rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <p className="mb-4 text-xs font-black uppercase tracking-[0.2em] text-slate-400">Certifications</p>
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={item.cert} className="flex items-start justify-between gap-3 text-sm">
                        <div>
                          <span className="font-bold text-slate-700">{item.cert}</span>
                          <span className="block text-xs text-slate-400">{item.org}</span>
                        </div>
                        <span className="flex-shrink-0 rounded-full border border-cyan-200 bg-cyan-50 px-2 py-0.5 text-xs font-black text-cyan-600">Current</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="flex flex-col gap-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Fitness Equipment Repair', 'Process Improvement', 'Asset Management', 'Six Sigma', 'Scrum', 'Automation', 'Commercial Gym Operations', 'Residential Service'].map((tag) => (
                    <span key={tag} className="rounded-full border border-slate-200 bg-white px-4 py-1.5 text-xs font-black uppercase tracking-[0.14em] text-slate-500 shadow-sm">{tag}</span>
                  ))}
                </div>

                {/* Recommendations */}
                <div className="space-y-4">
                  <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-400">LinkedIn Recommendations</p>

                  <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p className="text-sm leading-relaxed text-slate-600 italic">"Robby Turner is an exceptional professional whose commitment to excellence and client satisfaction is truly impressive. Working with 2EZ TEK has been an outstanding experience. His communication skills are impeccable, and his ability to provide quality service for home gym repair and assembly has been remarkable. I highly recommend Robby and 2EZ TEK for anyone seeking reliable, high-quality gym equipment services."</p>
                    <div className="mt-4 flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full border border-slate-200 bg-slate-100 flex items-center justify-center text-xs font-black text-slate-400">GS</div>
                      <div>
                        <p className="text-sm font-black text-slate-700">Gina Smith</p>
                        <p className="text-xs text-slate-400">Client Recommendation</p>
                      </div>
                    </div>
                  </div>

                  <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p className="text-sm leading-relaxed text-slate-600 italic">"Robby is a forward-thinking entrepreneur with a genuine passion for delivering excellent service. His approach to fitness equipment repair combines technical expertise with a strong understanding of customer needs. The work 2EZ TEK does is a direct reflection of Robby's drive to provide professional, reliable service that clients can count on."</p>
                    <div className="mt-4 flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full border border-slate-200 bg-slate-100 flex items-center justify-center text-xs font-black text-slate-400">EE</div>
                      <div>
                        <p className="text-sm font-black text-slate-700">Ernie Elbert</p>
                        <p className="text-xs text-slate-400">Client Recommendation</p>
                      </div>
                    </div>
                  </div>
      ...
      <section className="border-t border-slate-200 bg-white px-6 py-28 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-14 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="flex items-center gap-3 mb-5">
                <span className="h-px w-8 bg-cyan-400" />
                <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">Google Reviews</span>
              </div>
              <h2 className="text-4xl font-black leading-tight md:text-5xl">
                500+ Five-Star Reviews
                <span className="block text-slate-400">Across Dallas Fort Worth.</span>
              </h2>
            </div>
            <a
              ...
              className="self-start flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-6 py-3.5 text-xs font-black uppercase tracking-[0.15em] text-slate-700 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50"
            >
          ...
          <motion.div variants={staggerContainer(0.06, 0.05)} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {featuredReviews.map((item) => (
              <motion.div key={item.name + item.title} variants={staggerItem} whileHover={{ y: -5 }} transition={{ duration: 0.3, ease: EASE }} className="flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-colors hover:border-cyan-200">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm text-cyan-400">★★★★★</span>
                  <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">{item.tag}</span>
                </div>
                <p className="flex-1 text-sm leading-7 text-slate-600">"{item.review}"</p>
                <div className="mt-5 flex items-end justify-between border-t border-slate-200 pt-4">
                  <div>
                    <div className="text-sm font-black text-cyan-600">{item.name}</div>
          ...
          <Reveal delay={0.2} className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <Link href="/reviews" className="rounded-2xl border border-slate-200 bg-white px-7 py-4 text-sm font-black uppercase tracking-[0.12em] text-slate-700 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50">
              View All {googleReviews.length} Reviews
            </Link>
            <a href={GOOGLE_REVIEW_URL} target="_blank" rel="noopener noreferrer" className="button-glow rounded-2xl bg-cyan-400 px-7 py-4 text-sm font-black uppercase tracking-[0.12em] text-black transition hover:scale-105">
              Leave a Review
            </a>
          </Reveal>
        </div>
      ...
      <section className="border-t border-slate-200 bg-slate-50 px-6 py-28 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-14 max-w-3xl">
            <div className="flex items-center gap-3 mb-6">
              <span className="h-px w-8 bg-cyan-400" />
              <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">What We Do</span>
            </div>
            <h2 className="text-4xl font-black leading-tight md:text-6xl">
              From Home Gyms
              <span className="block text-slate-400">To Commercial Facilities.</span>
            </h2>
            <p className="mt-6 text-lg leading-relaxed text-slate-500">Our work covers the full spectrum of fitness equipment service: repair, assembly, maintenance, and installation for residential and commercial clients across DFW.</p>
          </Reveal>
          ...
          <motion.div variants={staggerContainer(0.07, 0.1)} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((service, i) => (
              <motion.div key={service.title} variants={staggerItem} whileHover={{ y: -6 }} transition={{ duration: 0.3, ease: EASE }} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:border-cyan-200 hover:bg-cyan-50">
                <div className="flex items-center gap-3 mb-3">
                  <span className="h-px w-5 bg-cyan-400" />
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-cyan-600">Service {String(i + 1).padStart(2, '0')}</span>
                </div>
          ...
          <Reveal delay={0.2} className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <Link href="/brands" className="rounded-2xl border border-slate-200 bg-white px-7 py-4 text-sm font-black uppercase tracking-[0.12em] text-slate-700 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50">Brands We Service</Link>
          </Reveal>
        </div>
      </section>
      ...
      <section className="border-t border-slate-200 bg-white px-6 py-28 lg:px-16">
        <div className="mx-auto max-w-7xl grid gap-16 lg:grid-cols-[1fr,1.2fr] lg:items-start">
          <Reveal direction="left">
            <div className="flex items-center gap-3 mb-6">
              <span className="h-px w-8 bg-cyan-400" />
              <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">Our Difference</span>
            </div>
            <h2 className="text-4xl font-black leading-tight md:text-6xl">
              Real Technicians.
              <span className="block text-slate-400">Smarter Systems.</span>
            </h2>
            <p className="mt-6 text-lg leading-relaxed text-slate-600">
            ...
            <div className="mt-10 flex flex-wrap items-center gap-3">
              ...
              <a href="tel:9728077232" className="rounded-2xl border border-slate-200 bg-white px-7 py-4 text-sm font-black uppercase tracking-[0.12em] text-slate-700 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50">Call (972) 807-7232</a>
            </div>
          </Reveal>
          ...
          <motion.div variants={staggerContainer(0.1, 0.1)} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-60px' }} className="grid gap-4">
            {values.map((v) => (
              <motion.div key={v.title} variants={staggerItem} whileHover={{ x: 6 }} transition={{ duration: 0.3, ease: EASE }} className="flex items-start gap-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:border-cyan-200">
                <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl border border-cyan-200 bg-cyan-50 text-xl">{v.icon}</div>
                <div>
      ...
      <section className="border-t border-slate-200 bg-slate-50 px-6 py-28 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <Reveal className="text-center max-w-3xl mx-auto mb-14">
            <div className="flex items-center justify-center gap-3 mb-6">
              <span className="h-px w-8 bg-cyan-400" />
              <span className="text-xs font-black uppercase tracking-[0.3em] text-cyan-600">Where We Work</span>
              <span className="h-px w-8 bg-cyan-400" />
            </div>
            <h2 className="text-4xl font-black leading-tight md:text-6xl">
              Serving All of Dallas
              <span className="block text-slate-400">Fort Worth.</span>
            </h2>
            <p className="mt-6 text-lg leading-relaxed text-slate-500">From Addison to Arlington, Frisco to Fort Worth. 2EZ TEK covers the full DFW metroplex for residential and commercial fitness equipment service.</p>
          </Reveal>

          <Reveal delay={0.2} className="flex flex-wrap justify-center gap-3">
            {areas.map((area) => (
              <Link key={area} href={'/areas/' + area.toLowerCase().replace(' ', '-')} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-slate-600 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-600">
                {area}
              </Link>
            ))}
          </Reveal>
