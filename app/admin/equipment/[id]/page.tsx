...
export default function EquipmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function loadNotes() {
    fetch(`/api/admin/equipment/${id}/notes`, {
      headers: { 'x-admin-password': getPassword() },
    })
      .then((r) => r.json())
  ...
  async function addNote() {
    ...
    setAddingNote(true)
    try {
      const res = await fetch(`/api/admin/equipment/${id}/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-password': getPassword() },
        body: JSON.stringify({ note: noteText.trim(), visible_to_customer: noteVisible }),
      })
  ...
  useEffect(() => { loadData(); loadNotes() }, [id])
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 px-6 py-4">
                <h2 className="font-black text-slate-900">Tech Notes</h2>
                <p className="text-xs text-slate-500">Internal updates, optionally shared with the customer</p>
              </div>
              ...
              <div className="border-b border-slate-100 px-6 py-4">
                <textarea
                  ...
                  rows={3}
                  placeholder="e.g. Replaced brake assembly, tested resistance — working normally."
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-900 outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100 resize-none"
                />
                <div className="mt-3 flex items-center justify-between">
                  <label className="flex items-center gap-2 text-xs font-medium text-slate-600">
                    <input
                      ...
                      className="h-3.5 w-3.5 rounded border-slate-300 accent-cyan-500"
                    />
                    Visible to customer
                  </label>
                  <button
                    ...
                    className="rounded-lg bg-slate-900 px-4 py-2 text-xs font-black uppercase tracking-widest text-white transition hover:bg-slate-700 disabled:opacity-50"
                  >
                  ...
                  {notes.map((n) => (
                    <div key={n.id} className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wide ${n.visible_to_customer ? 'bg-cyan-100 text-cyan-700' : 'bg-slate-100 text-slate-500'}`}>
                          {n.visible_to_customer ? 'Customer visible' : 'Internal only'}
                        </span>
