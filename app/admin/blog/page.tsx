...
function BulkGenerateButton({ password, onDone }: { password: string; onDone: () => void }) {
  ...
  return (
    <div className="mb-8 rounded-[1.5rem] border border-cyan-400/20 bg-cyan-400/5 p-6">
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div>
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-cyan-300">Generate Series</h2>
          <p className="mt-1 text-xs text-white/40">Generate multiple articles for a specific brand</p>
        </div>
        <div className="ml-auto flex items-center gap-3">
          <input
            value={brand}
            ...
            className="w-36 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-center text-sm font-bold text-white outline-none focus:border-cyan-400"
          />
          <button
            onClick={run}
            disabled={running}
            className="rounded-2xl bg-cyan-400 px-6 py-2.5 text-xs font-black uppercase tracking-[0.14em] text-black transition hover:bg-cyan-300 disabled:opacity-50"
          >
